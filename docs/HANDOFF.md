...
- 수입 예정, 지출 예정, 중요한 일정, 보관 메모를 한 앱에서 관리
...
- 홈 탭: 오늘의 주요 항목, 이번 달 흐름, 다가오는 일정/보관 메모
- 일정 탭: 월간/주간 캘린더, 오늘 요약, 일정/기념일 CRUD, 일정 메모
- 흐름 탭: 수입 예정, 지출 예정, 날짜별 반복 수입/지출, 기록, 정기 항목, 계산기
- 금고 탭: 중요 메모, 재정 메모, 계약/정리, 갱신/만료, 집/차량 기록
- 생활 탭: 장보기, 체크리스트, 용품, 집안일, 템플릿 기능은 유지하되 하단 탭에서는 숨김
...
- 민감 메모 PIN: 앱 전체 잠금은 사용하지 않음. 설정에서 4자리 PIN을 정하면 `민감`, `비밀`, `숨김`, `private` 태그가 붙은 보관 메모를 열 때만 한 번 확인함. PIN은 새 4자리로 간단히 바꾸거나 끌 수 있음
- 금고 보호: 설정에서 `민감 메모 숨김`을 켜면 `민감`, `비밀`, `숨김`, `private` 태그가 붙은 보관 메모의 제목/내용을 가림
- 민감 메모 열람: 민감 보관 메모 숨김이 켜져 있고 PIN이 설정된 경우, 금고 목록에서 민감 항목을 열 때 4자리 PIN을 확인
- 민감 메모 PIN 안내: PIN 없이 `민감 메모 숨김`을 켜면 설정 화면에서 PIN 안내를 표시하고 민감 메모 PIN 설정 영역을 자동으로 열어 이어서 설정할 수 있음
- 금고 템플릿: 새 보관 메모 작성 시 계좌 잔액, 주식/코인, 보험/계약, 대출/카드, 갱신/만료, 비상정보 템플릿으로 제목/내용/유형/태그를 빠르게 채울 수 있음
- 금고 찾기: 금고 목록에 `중요만`, `민감만` 빠른 필터를 추가하고, 홈의 중요 금고 노출 기준을 계좌/보험/계약/투자/대출/카드/갱신/만료/비상 태그까지 확장
- 금고 날짜: 갱신/만료 유형 또는 갱신/만료 태그가 붙은 보관 메모는 앞으로 30일 안에 해당하면 홈의 `다가오는 항목`에 날짜와 함께 표시
- 흐름 요약: 흐름 탭에서 수입 예정을 월급/정기, 부가 수입, 기타 반복, 이번 달 기록으로 나누고 지출 예정을 카드 결제, 정기 지출, 자동결제, 이번 달 기록으로 분리 표시
- 흐름 일정: 현재 달의 날짜별 흐름은 오늘 이후 항목이 먼저 보이고 `오늘`, `D-n`, `지남` 상태 배지를 표시
- 흐름 일정 초과 항목: 날짜별 흐름이 8건을 넘으면 숨겨진 건수를 표시하고 `더 보기/접기`로 전체 정기 항목 흐름을 확인 가능
- 남은 흐름: 현재 달 흐름 카드에 오늘 이후 남은 수입, 남은 지출, 예상 차액을 별도 표시
- 정기 항목 월 이동: `정기 항목` 탭에서도 월 이동 바를 표시해 수입 예정/지출 예정/자동결제의 월별 상태를 어느 달 기준으로 처리하는지 명확히 함
- 정기 항목 상태 요약: 수입 예정/지출 예정/자동결제 관리 탭 상단은 전체 금액이 아니라 선택한 달의 남은 금액을 주 숫자로 표시하고, 전체 금액과 처리 건수를 보조 문구로 표시
- 수입 예정 확인: 월급/부가 수입 같은 반복 수입은 월별로 `입금/취소` 처리할 수 있고, 날짜가 지난 입금 대기 항목은 흐름 카드에 경고로 표시
- 미납 반영: 완료 처리된 정기 지출은 남은 지출에서 제외하고, 날짜가 지난 미완료 정기 지출은 미납 경고와 날짜별 흐름 상단에 표시. 날짜별 흐름에서 정기 지출을 바로 완료/취소 처리 가능
...
- 홈 흐름 경고: 홈의 `오늘의 주요 항목`은 월별 상태를 읽어 입금 대기, 미납, 자동결제 확인 필요 항목을 오늘 예정 항목보다 먼저 표시
- 홈 흐름 요약: 홈의 `이번 달 흐름`은 전체 예상액이 아니라 월별 상태를 반영한 남은 수입, 남은 지출, 예상 차액으로 표시
- 홈 즉시 처리: 홈의 흐름 항목에서 `입금`, `완료`, `결제` 버튼으로 흐름 탭 이동 없이 월별 상태를 바로 처리 가능
- 다가오는 항목: 홈의 `다가오는 항목`은 앞으로 7일 안의 미처리 수입 예정, 지출 예정, 자동결제를 월별 상태 기준으로 먼저 표시. 월말에는 다음 달 초 정기 항목도 함께 계산
- 홈 초과 항목: `오늘의 주요 항목`이 5건을 넘으면 숨겨진 건수를 표시하고 흐름 탭으로 이동할 수 있는 버튼을 제공
- 월말 날짜 보정: 매월 29~31일 정기 항목은 해당 월에 그 날짜가 없으면 월 마지막 날로 계산. 홈, 흐름 탭, 캘린더 자동 집계가 같은 기준을 사용
- 모바일 사용성: 홈 첫 화면을 `오늘의 주요 항목 → 빠른 추가 → 이번 달 흐름 → 다가오는 항목` 흐름으로 정리. 빠른추가는 `지출 예정`, `수입 예정`, `중요 일정`, `보관 메모` 중심으로 정리
...
- 입력 UX: 흐름/일정/금고 추가는 핵심 필드만 먼저 보이고 날짜/담당자/반복 등은 `자세히` 안에 배치
- 입력 폼 2차 단순화: 일정/장보기는 추천 버튼과 Enter 저장을 지원. 체크리스트/기록/정기 지출/구독은 필수 입력만 먼저 보이고 마감일, 담당자, 결제수단, 태그, 메모 등은 `자세히` 안에 배치
...
- 반복 QA 명령: `npm run smoke`로 빌드 산출물, 빌드 asset 연결, PWA manifest, 필수 아이콘, OG 메타, 이전 용어 잔여 여부를 자동 확인. `npm run verify`는 env:check, analytics:check, typecheck, build, smoke를 순서대로 실행
...
- 실사용 테스트 체크리스트: `docs/TEST_CHECKLIST.md`를 현재 홈/흐름/일정/금고 중심 구조, 민감 메모 PIN, 백업 교체, 설치 안내 흐름 기준으로 최신화
- 설정 저장 갱신: 설정에서 집 이름을 저장하면 헤더의 집 이름도 탭 이동 없이 즉시 갱신됨
- 생활 저장 이동: 빠른 추가에서 장보기나 체크리스트를 저장하면 숨겨진 생활 화면의 해당 목록으로 바로 이동
- 빠른 추가 이동: 빠른 추가 저장 후 수입/지출/정기 지출/자동결제는 흐름 탭, 일정은 일정 탭, 보관 메모는 금고 탭, 구매 항목/체크리스트는 생활 화면으로 이동해 저장 결과를 바로 확인할 수 있음
- 흐름 입력 개선: 지출/수입 금액 입력에서 Enter 저장을 지원하고, 수입 입력 시 월급/부가 수입에 맞는 큰 금액 추천 버튼을 표시
- 문구 고급화: 사용자 노출 문구는 쉬운 표현을 유지하되 `나갈 돈/들어올 돈/살 것/금고 메모` 대신 `지출 예정/수입 예정/구매 항목/보관 메모`를 기본 용어로 사용
...
- `hide_sensitive_records`: `true`이면 민감 태그 보관 메모 제목/내용 숨김
