...
- 홈 탭: 오늘의 주요 항목, 빠른 추가, 이번 달 가계부, 다가오는 일정/메모
- 일정 탭: 월간/주간 캘린더, 날짜별 일정 제목 미리보기, 일정/기념일 CRUD
- 가계부 탭: 수입, 지출, 날짜별 반복 수입/지출, 상세내역, 입출금, 계산기
- 메모장 탭: 중요 메모, 재정 메모, 투자 메모, 계약/정리, 갱신/만료, 집/차량 기록
...
- 메모장 비밀 표시: 메모장 내용 입력칸의 `공개/비밀` 중 `비밀`을 선택하면 별도 설정 없이 목록에서 제목만 보이고 내용은 가림. 메모를 누르면 PIN 없이 바로 열람/수정 가능
- 비밀 메모 호환: 기존 `민감`, `비밀`, `숨김`, `private` 태그 메모는 과거 데이터 호환용으로 비밀 내용으로 판정하되, 새 메모는 태그 입력 없이 내용 옆 토글로 처리함
- 비밀 메모 열람: PIN 확인 없이 바로 열람/수정 가능. 비밀 기능은 혼자 쓰는 앱에서 목록 내용만 가리는 용도로 유지
- 메모장 추가 UX: 상단 `메모 남기기` 카드에 `+` 표시를 넣고, 메모 유형 버튼은 가로 스크롤 없이 여러 줄로 줄바꿈
- 메모장 입력: 새 메모는 제목/내용/태그가 모두 빈칸에서 시작하며, 계좌/보험/대출 같은 자동 템플릿 문구는 넣지 않음
- 메모장 찾기: 메모장 목록은 검색으로 찾고, 홈의 중요 메모 노출 기준은 계좌/보험/계약/투자/대출/카드/갱신/만료/비상 태그까지 확장
...
- 가계부 상세내역: 선택한 달의 지출/수입 카테고리 상위 항목을 가로 막대그래프로 표시해 월간 흐름을 먼저 확인 가능
...
- 모바일 사용성: 홈 첫 화면을 `오늘의 주요 항목 → 빠른 추가 → 이번 달 가계부 → 다가오는 항목` 흐름으로 정리. 빠른추가는 `지출 예정`, `수입 예정`, `체크리스트`, `중요 일정`을 우선 노출하고 `메모장`, `정기 지출`, `구독/자동결제`를 함께 제공
...
- 생활 기능 단순화: 하단 `생활` 탭을 `체크리스트`로 변경하고 홈의 체크리스트 목록 노출은 제거
...
- QA 보완: 메모장 입력은 템플릿 자동 삽입 없이 빈 제목/내용에서 시작하도록 정리
- 실사용 테스트 체크리스트: `docs/TEST_CHECKLIST.md`를 현재 홈/가계부/일정/체크리스트/메모장 구조, 메모장 비밀 표시, 백업 교체, 설치 안내 흐름 기준으로 최신화
...
- 가계부 입력 개선: 상단 `수입기록`/`지출기록` 카드에서 바로 입력 가능. 지출/수입 금액 입력에서 Enter 저장을 지원하고, 수입 입력 시 월급/부가 수입에 맞는 큰 금액 추천 버튼을 표시
...
- 메모장 단순화: 메모장 검색창 아래는 유형별 빠른 추가 한 줄만 남기고 `전체/중요만/비밀만` 필터 줄을 제거해 화면을 더 단순하게 정리
- 2026-05-26 피드백 반영: 메모장에 `투자 메모` 유형을 복구하고, 홈 체크리스트 노출 제거. 가계부 탭 상단 `수입기록`/`지출기록` 카드는 유지하고, 상세내역 중간 우측의 `CSV`, `+ 지출`, `+ 수입` 중복 버튼 제거. 체크리스트 긴 문장은 줄바꿈으로 전체 표시
...
- `app_lock_pin_hash`: 과거 비밀 메모 PIN 해시. 현재 UI/열람에는 사용하지 않음
- `app_lock_pin_salt`: 과거 비밀 메모 PIN salt. 현재 UI/열람에는 사용하지 않음
- `hide_sensitive_records`: 과거 전역 비밀 내용 숨김 설정 키. 현재 설정 UI에서는 제거했고, 메모장은 각 메모의 `content_is_secret` 값만으로 목록 내용을 가림
...
탭별 빠른 메모의 비밀 여부는 `onzip_tab_memo_secrets`에 별도로 저장한다.
...
3. 7일 실사용 관찰
   ...
   - 특히 `지출 예정`, `수입 예정`, `메모`, `백업 파일로 교체하기`, `메모장 비밀 표시` 흐름을 실제 사용자에게 설명 없이 맡겨보기
