...
- 홈 탭: 오늘 확인, 자주 쓰는 빠른 추가 4개, 월간 요약 중심으로 단순화
...
- 온보딩/업데이트: 첫 실행 안내와 설정의 최근 업데이트 카드 추가
...
- 모바일 사용성: 홈 첫 화면을 `오늘 확인 → 바로 추가 → 이번 달 상태` 흐름으로 더 단순화. 설정은 헤더로 이동하고 메모는 하단 탭에 배치. 빠른추가는 `돈 썼어요`, `살 것 추가`, `일정 넣기`, `메모 남기기`처럼 행동 중심 문구로 정리. 바텀시트 폼은 `100dvh`/safe-area/큰 터치 영역/하단 고정 저장 버튼 기준으로 정리
...
1. 모바일 실사용 흐름 다듬기
   - 변경된 홈/가계부/장보기 구조를 실제 사용자에게 다시 테스트
