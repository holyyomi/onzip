...
- 흐름 일정 초과 항목: 날짜별 돈 흐름이 8건을 넘으면 숨겨진 건수를 표시하고 `더 보기/접기`로 전체 반복돈 흐름을 확인 가능
- 남은 흐름: 현재 달 흐름 카드에 오늘 이후 남은 받을 돈, 남은 나갈 돈, 남은 차이를 별도 표시
- 반복돈 월 이동: `반복돈` 탭에서도 월 이동 바를 표시해 받을 돈/나갈 돈/자동결제의 월별 상태를 어느 달 기준으로 처리하는지 명확히 함
- 반복돈 상태 요약: 받을 돈/나갈 돈/자동결제 관리 탭 상단은 전체 금액이 아니라 선택한 달의 남은 금액을 주 숫자로 표시하고, 전체 금액과 처리 건수를 보조 문구로 표시
...
- 자동결제 확인: 구독/자동결제는 월별 `결제됨/취소` 처리와 결제 확인 필요 경고를 지원. 완료된 자동결제는 남은 나갈 돈에서 제외
...
- 홈 흐름 요약: 홈의 `이번 달 흐름`은 전체 예상액이 아니라 월별 상태를 반영한 남은 받을 돈, 남은 나갈 돈, 남은 차이로 표시
- 홈 즉시 처리: 홈의 돈 항목에서 `받음`, `완료`, `결제` 버튼으로 흐름 탭 이동 없이 월별 상태를 바로 처리 가능
- 곧 챙길 돈: 홈의 `곧 챙길 것`은 앞으로 7일 안의 미처리 받을 돈, 나갈 돈, 자동결제를 월별 상태 기준으로 먼저 표시. 월말에는 다음 달 초 반복돈도 함께 계산
- 홈 초과 항목: `오늘 중요한 것`이 5건을 넘으면 숨겨진 건수를 표시하고 흐름 탭으로 이동할 수 있는 버튼을 제공
- 월말 날짜 보정: 매월 29~31일 반복돈은 해당 월에 그 날짜가 없으면 월 마지막 날로 계산. 홈, 흐름 탭, 캘린더 자동 집계가 같은 기준을 사용
