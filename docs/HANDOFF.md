...
- 가계부 입력 개선: 상단 `지출기록`/`수입기록` 카드에서 바로 입력 가능. 지출/수입 금액 입력에서 Enter 저장을 지원하고, 수입 입력 시 월급/부가 수입에 맞는 큰 금액 추천 버튼을 표시
...
- 2026-05-26 피드백 반영: 메모장에 `투자 메모` 유형을 복구하고, 홈 체크리스트 노출 제거. 가계부 탭 상단 `지출기록`/`수입기록` 카드는 유지하고, 상세내역 중간 우측의 `CSV`, `+ 지출`, `+ 수입` 중복 버튼 제거. 체크리스트 긴 문장은 줄바꿈으로 전체 표시
