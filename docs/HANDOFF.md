...
- 민감 메모 열람: 민감 보관 메모 숨김이 켜져 있고 PIN이 설정된 경우, 금고 목록에서 민감 항목을 열 때 PIN 확인을 먼저 요구
- 민감 메모 PIN 안내: PIN 없이 `민감 메모 숨김`을 켜면 설정 화면에서 PIN 필요 안내를 표시하고 앱 전체 잠금 설정 영역을 자동으로 열어 보안 설정을 이어갈 수 있음
