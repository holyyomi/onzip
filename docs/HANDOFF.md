...
- 금액 보호: 설정에서 `금액 가리기`를 켜면 홈, 흐름, 일정, 금고의 주요 금액이 `***원`으로 표시됨
- 앱 잠금: 설정에서 숫자 4~6자리 PIN을 설정하면 앱 시작 시 잠금 화면 표시. PIN 원문은 저장하지 않고 salt+SHA-256 해시만 저장. 5분 미사용 또는 1분 이상 백그라운드 후 복귀 시 자동 재잠금
...
2. 금고 보안성 보완
   - 계산기/CSV 내보내기 등 보조 기능의 금액 노출 정책 결정
   - PIN 분실 시 복구/초기화 UX 검토
