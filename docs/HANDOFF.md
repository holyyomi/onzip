...
- 비밀 내용 숨김: 설정에서 `비밀 내용 숨김`을 켜면 각 입력칸의 `공개/비밀` 선택 중 `비밀`로 저장한 내용만 가림. 메모장은 제목은 그대로 보이고 내용만 가림
...
- 실사용 테스트 체크리스트: `docs/TEST_CHECKLIST.md`를 현재 홈/가계부/일정/체크리스트/메모장 구조, 비밀 내용 숨김, 백업 교체, 설치 안내 흐름 기준으로 최신화
...
3. 7일 실사용 관찰
   ...
   - 특히 `지출 예정`, `수입 예정`, `메모`, `백업 파일로 교체하기`, `비밀 내용 숨김` 흐름을 실제 사용자에게 설명 없이 맡겨보기
