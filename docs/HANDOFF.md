...
- 비밀 내용 숨김: 설정에서 `비밀 내용 숨김`을 켜면 각 입력칸의 `비밀` 토글이 켜진 내용만 가림. 메모장은 제목은 그대로 보이고 내용만 가림
- 비밀 메모 호환: 기존 `민감`, `비밀`, `숨김`, `private` 태그 메모는 과거 데이터 호환용으로 비밀 내용으로 판정하되, 새 메모는 태그 입력 없이 내용 옆 토글로 처리함
- 비밀 메모 PIN: 앱 전체 잠금은 사용하지 않음. 설정에서 4자리 PIN을 정하면 비밀 메모장 항목을 열 때만 한 번 확인함. PIN은 새 4자리로 간단히 바꾸거나 끌 수 있음
- 비밀 메모 PIN 안내: PIN 없이 `비밀 내용 숨김`을 켜면 설정 화면에서 PIN 안내를 표시하고 비밀 메모 PIN 설정 영역을 자동으로 열어 이어서 설정할 수 있음
...
- 실사용 테스트 체크리스트: `docs/TEST_CHECKLIST.md`를 현재 홈/가계부/일정/체크리스트/메모장 구조, 비밀 메모 PIN, 백업 교체, 설치 안내 흐름 기준으로 최신화
...
- `app_lock_pin_hash`: 비밀 메모 PIN 해시
- `app_lock_pin_salt`: 비밀 메모 PIN salt
- `hide_sensitive_records`: `true`이면 비밀 토글이 켜진 내용 숨김. 기존 설정 키 이름은 호환을 위해 유지
...
3. 7일 실사용 관찰
   ...
   - 특히 `지출 예정`, `수입 예정`, `메모`, `백업 파일로 교체하기`, `비밀 메모 PIN` 흐름을 실제 사용자에게 설명 없이 맡겨보기
