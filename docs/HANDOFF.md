...
- 빠른 추가: 저장 후 홈/흐름/일정/생활/금고 화면 즉시 갱신, 완료 알림 표시
...
- 모바일 사용성: 홈 첫 화면을 `오늘의 주요 항목 → 빠른 추가 → 생활 할 일 → 이번 달 흐름 → 다가오는 항목` 흐름으로 정리. 빠른추가는 `지출 예정`, `구매 항목`, `체크리스트`, `중요 일정`을 우선 노출하고 `수입 예정`, `보관 메모`, `정기 지출`, `구독/자동결제`를 함께 제공
...
- 실사용 테스트 체크리스트: `docs/TEST_CHECKLIST.md`를 현재 홈/흐름/일정/생활/금고 구조, 민감 메모 PIN, 백업 교체, 설치 안내 흐름 기준으로 최신화
- 설정 저장 갱신: 설정에서 집 이름을 저장하면 헤더의 집 이름도 탭 이동 없이 즉시 갱신됨
- 생활 저장 이동: 빠른 추가에서 구매 항목이나 체크리스트를 저장하면 생활 화면의 해당 목록으로 바로 이동
