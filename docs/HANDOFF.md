...
- 앱 잠금: 설정에서 숫자 4~6자리 PIN을 설정하면 앱 시작 시 잠금 화면 표시. PIN 원문은 저장하지 않고 salt+SHA-256 해시만 저장. 5분 미사용 또는 1분 이상 백그라운드 후 복귀 시 자동 재잠금. PIN 우회는 제공하지 않고 잠금 화면/설정에 백업 안내 표시
...
2. 금고 보안성 보완
   - PIN 분실 시 브라우저 데이터 삭제/백업 복원 안내 문구 추가 보완
